...
As Census Bureau changes how [TIGER/Line shapefiles](https://www.census.gov/programs-surveys/geography/technical-documentation/complete-technical-documentation/file-availability.2010.html#list-tab-2064248757) are released, state-based ZCTA data are no longer available since 2011, while nation-based data remain available. In turn, this affect obtaining data from ACS at the ZCTA level for a specific state using [tidycensus]((https://walker-data.com/tidycensus/), which uses Census shapefiles to request geometry. According to a relevant [github issue](https://github.com/walkerke/tidycensus/issues/78), [tidycensus]uses 2010 ZCTA file (from [tigris package](https://cran.r-project.org/web/packages/tigris/index.html)) to obtain state-specific ZCTA-level data in the ACS for 2011-2018. Starting 2019, using [tidycensus::get_acs()] to retrieve state-specific ZCTA-level data will produce an error. The 3 datasets of ZCTA-to-state crosswalks are included to help selecting the ZCTAs in the state(s) of interest after retrieving the ZCTA data at the national level.    
