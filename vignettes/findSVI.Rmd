...
For SVI table output, `summarise_svi()` has options to either return a full SVI table with every SVI variable and intermediate ranking values (as `get_svi()`), or return a summarised SVI table with only the GEOID, theme-specific SVIs and SVI for all 4 themes for each year-state combinations. For both options, corresponding year and state information will be included as two separate columns in the table.
